import { ALGORITHM_VERSION, THRESHOLDS } from "@/core/constants/health";
import type { DailyCheckin, MedicalResult, RiskAssessment } from "@/models";
...
export interface DetectedPattern {
  ...
  category: string;
  detail: string;
  severity: 0 | 1 | 2; // 0 info, 1 moderate, 2 high
}

const asc = (checkins: DailyCheckin[]) =>
  [...checkins].sort((a, b) => (toDate(a.date)?.getTime() ?? 0) - (toDate(b.date)?.getTime() ?? 0));

const series = (checkins: DailyCheckin[], key: keyof DailyCheckin): number[] =>
  asc(checkins)
    .map((c) => c[key])
    .filter((v): v is number => typeof v === "number" && !Number.isNaN(v));

export function detectPatterns(checkins: DailyCheckin[], verifiedResults: MedicalResult[] = []): DetectedPattern[] {
  ...
  if (sleep.length >= t.checkin.minCheckinsForTrend) {
    ...
    if (window.length >= 3 && (window[0]! - window[window.length - 1]!) >= t.sleep.declineDeltaHours) {
      patterns.push({
    ...
    if (avg < t.sleep.veryLowHours) {
      patterns.push({ factor: "short_sleep", category: "sleep", detail: `Average sleep is ${avg.toFixed(1)}h.`, severity: 2 });
    } else if (avg < t.sleep.lowHours) {
      patterns.push({ factor: "short_sleep", category: "sleep", detail: `Average sleep is ${avg.toFixed(1)}h.`, severity: 1 });
    }
  }
  ...
  if (ex.length >= t.checkin.minCheckinsForTrend) {
    ...
    if (avg === 0) {
      patterns.push({ factor: "no_activity", category: "exercise", detail: "No activity logged in recent entries.", severity: 2 });
    } else if (avg < t.exercise.inactiveDailyMinutes) {
      patterns.push({ factor: "low_activity", category: "exercise", detail: `Average ${avg.toFixed(0)} min/day recently.`, severity: 1 });
    }
  }
  ...
  if (water.length >= t.checkin.minCheckinsForTrend) {
    ...
    if (avg < t.water.lowGlasses) {
      patterns.push({ factor: "low_hydration", category: "water", detail: `Average ${avg.toFixed(1)} glasses/day.`, severity: 1 });
    }
  }
  ...
  if (highBp.length >= 2) {
    const severe = highBp.some(
      (c) => (c.systolicBP ?? 0) >= t.bloodPressure.highSystolic || (c.diastolicBP ?? 0) >= t.bloodPressure.highDiastolic,
    );
    patterns.push({
  ...
  /* food quality */
  const foods = asc(checkins).slice(-5).map((c) => c.foodQuality).filter(Boolean);
  if (foods.length >= 3 && foods.filter((f) => f === "poor" || f === "fair").length >= foods.length - 1) {
    patterns.push({ factor: "poor_food_quality", category: "food", detail: "Most recent meals were logged as poor or fair.", severity: 1 });
  }
...
export interface HealthScore {
  score: number;
  band: "needs_attention" | "fair" | "good" | "excellent";
 ...
 */
export function calculateHealthScore(checkins: DailyCheckin[], patterns: DetectedPattern[]): HealthScore {
  const contributions: Array<{ label: string; delta: number }> = [];
  let score = 100;

  if (checkins.length === 0) {
    return { score: 0, band: "needs_attention", contributions: [{ label: "No check-ins recorded yet", delta: 0 }] };
  }
  ...
  const recentDays = checkins.filter(
    (c) => (toDate(c.date)?.getTime() ?? 0) >= Date.now() - THRESHOLDS.checkin.consistencyWindowDays * 86400000,
  ).length;
  const consistencyBonus = Math.min(10, recentDays * 2);
  score += consistencyBonus;
  contributions.push({ label: `${recentDays} check-ins in the last week`, delta: consistencyBonus });

  score = Math.max(0, Math.min(100, Math.round(score)));
  const band = score >= 85 ? "excellent" : score >= 70 ? "good" : score >= 50 ? "fair" : "needs_attention";
  return { score, band, contributions };
}

export function buildAssessments(patterns: DetectedPattern[], sourceRecordIds: string[]): RiskAssessment[] {
  const byCategory = new Map<string, DetectedPattern[]>();
  for (const p of patterns) {
...
export function suggestSpecialty(patterns: DetectedPattern[]): { specialty: string; urgency: string; basis: string } | null {
  ...
  if (factors.has("elevated_blood_pressure"))
    return { specialty: "Cardiology", urgency: high ? "soon" : "routine", basis: "Repeated blood pressure readings above the usual range." };
  if (factors.has("elevated_glucose"))
    return { specialty: "Endocrinology", urgency: high ? "soon" : "routine", basis: "Repeated glucose readings above the usual range." };
  if (factors.has("abnormal_verified_lab"))
    return { specialty: "General Physician", urgency: "routine", basis: "A verified lab value is outside its printed reference range." };
  if (factors.has("repeated_symptom"))
    return { specialty: "General Physician", urgency: "routine", basis: "The same symptom has been reported repeatedly." };
  if (factors.has("weight_increase_pattern") || factors.has("poor_food_quality"))
    return { specialty: "Nutrition & Dietetics", urgency: "routine", basis: "Weight and food-quality patterns in your check-ins." };
  if (factors.has("short_sleep") || factors.has("sleep_decline"))
    return { specialty: "General Physician", urgency: "routine", basis: "A sustained change in sleep duration." };
  return null;
}
