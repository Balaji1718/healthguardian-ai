...
import { dismiss, markRead, notificationPermission, requestNotificationPermission } from "@/services/notifications/notifications";
...
export const Route = createFileRoute("/app/notifications")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Reminders and context-aware alerts raised from your own health data." },
      { property: "og:title", content: "Your health notifications" },
      { property: "og:description", content: "Reminders and alerts raised from your own health data." },
    ],
  }),
});
...
function NotificationsPage() {
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        <EmptyState title="Nothing to show" description="Reminders and pattern alerts will appear here as you use the app." />
      ) : (
          ...
          {items.map((n) => (
            <li key={n.id} className="surface flex flex-wrap items-start gap-3 p-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-xs text-muted-foreground">{toDate(n.createdAt ?? null)?.toLocaleString() ?? ""}</p>
              </div>
              <Badge variant={n.priority === "high" ? "destructive" : "secondary"} className="capitalize">
                {n.priority}
              </Badge>
              {n.id && (
      ...
      <Disclaimer>
        Alerts appear while the app is open and, if you allow it, as browser notifications. HealthGuardian does not provide
        emergency monitoring — in an emergency contact local emergency services.
      </Disclaimer>
    </div>
  );
}
