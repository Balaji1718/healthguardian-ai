...
export const Route = createFileRoute("/app/goals")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Set realistic lifestyle goals and track progress from your own check-ins." },
      { property: "og:title", content: "Your health goals" },
      { property: "og:description", content: "Set realistic lifestyle goals and track progress over time." },
    ],
  }),
});
...
function GoalsPage() {
  ...
  const [form, setForm] = useState({ title: "", goalType: "exercise", targetValue: "", unit: "", frequency: "daily", targetDate: "" });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setOpen(false);
      setForm({ title: "", goalType: "exercise", targetValue: "", unit: "", frequency: "daily", targetDate: "" });
  ...
  return (
    <div>
      ...
      {open && (
        <form onSubmit={submit} className="surface mb-6 grid gap-4 p-6 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            ...
            <select id="goalType" className="h-9 w-full rounded-md border bg-background px-3 text-sm" value={form.goalType} onChange={(e) => setForm({ ...form, goalType: e.target.value })}>
              {GOAL_TYPES.map((t) => (
          ...
          <div className="space-y-1.5">
            ...
            <select id="frequency" className="h-9 w-full rounded-md border bg-background px-3 text-sm" value={form.frequency} onChange={(e) => setForm({ ...form, frequency: e.target.value })}>
          ...
          <div className="space-y-1.5">
            ...
            <Input id="targetValue" type="number" value={form.targetValue} onChange={(e) => setForm({ ...form, targetValue: e.target.value })} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="unit">Unit</Label>
            <Input id="unit" value={form.unit} onChange={(e) => setForm({ ...form, unit: e.target.value })} placeholder="minutes, glasses, kg" />
          </div>
      ...
      {goals.length === 0 ? (
        <EmptyState title="No goals yet" description="Create a goal to give your check-ins something to aim at." />
      ) : (
          ...
          {goals.map((g) => (
            <li key={g.id} className="surface p-5">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {g.frequency} · {g.targetValue != null ? `${g.progressValue}/${g.targetValue} ${g.unit ?? ""}` : "no numeric target"}
              </p>
              {g.targetValue != null && <Progress className="mt-3" value={Math.min(100, (g.progressValue / g.targetValue) * 100)} />}
              {g.status === "active" && g.id && (
                <Button variant="ghost" size="sm" className="mt-3 px-0" onClick={() => void complete(g.id!)}>
                  Mark completed
                </Button>
              )}
            </li>
