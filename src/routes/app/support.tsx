...
export const Route = createFileRoute("/app/support")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Raise a question or issue about your HealthGuardian AI account and data." },
      { property: "og:title", content: "HealthGuardian support" },
      { property: "og:description", content: "Raise a question or issue about your account and data." },
    ],
  }),
});
...
function SupportPage() {
  ...
  const [form, setForm] = useState({ type: "question", reason: "", message: "", priority: "normal" as "low" | "normal" | "high" });
  ...
  return (
    <div>
      <PageHeader title="Support" description="Account, data or app questions. For medical concerns, please contact a clinician." />

      <form onSubmit={submit} className="surface grid gap-4 p-6 sm:grid-cols-2">
        <div className="space-y-1.5">
          <Label htmlFor="type">Type</Label>
          <select id="type" className="h-9 w-full rounded-md border bg-background px-3 text-sm" value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}>
        ...
        <div className="sm:col-span-2">
          <Button type="submit" disabled={busy}>
            {busy ? <Loader2 className="mr-2 size-4 animate-spin" /> : <LifeBuoy className="mr-2 size-4" />} Submit request
          </Button>
        </div>
      </form>
      ...
      <section className="mt-8 space-y-2">
        {(data ?? []).map((r) => (
          <article key={r.id} className="surface flex flex-wrap items-center gap-3 p-4 text-sm">
            ...
            <span className="text-xs text-muted-foreground">{toDate(r.createdAt ?? null)?.toLocaleDateString() ?? ""}</span>
          </article>
        ))}
      </section>
