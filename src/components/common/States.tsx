...
export function ErrorState({ message, onRetry }: { message?: string; onRetry?: () => void }) {
  return (
    <div className="flex flex-col items-center gap-3 rounded-xl border border-destructive/30 bg-destructive/5 p-8 text-center">
      ...
      <p className="text-sm text-muted-foreground">{message ?? "Something went wrong while loading this section."}</p>
      {onRetry && (
...
export function OfflineNotice() {
  return (
    <div className="mb-4 flex items-center gap-2 rounded-lg border border-warning/40 bg-warning/10 px-3 py-2 text-sm">
      <WifiOff className="size-4" /> You are offline. Cached information is shown and new entries will sync when you reconnect.
    </div>
  );
}
