...
export async function runOcr(blob: Blob, mimeType: string, onProgress?: (p: number) => void): Promise<OcrOutcome> {
 ...
 */
export function extractResults(text: string, page: number, confidence: number | null): ExtractedCandidate[] {
  ...
  for (const line of lines) {
    ...
    const unit = m[3] ?? "";
    const tail = (m[4] ?? "").trim();
    if (!testName || /^(page|patient|name|age|sex|date|report|ref|doctor|lab)\b/i.test(testName)) continue;
