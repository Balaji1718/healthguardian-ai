...
export async function saveHealthProfile(uid: string, data: Partial<HealthProfile>) {
  await setDoc(healthProfileDoc(uid), clean({ ...data, updatedAt: serverTimestamp() }), { merge: true });
}
...
export async function listHealthRecords(uid: string, metric?: string, max = 200): Promise<HealthRecord[]> {
  const base = metric
    ? query(recordsCol(uid), where("metric", "==", metric), orderBy("recordedAt", "desc"), fbLimit(max))
...
export async function saveResult(uid: string, reportId: string, result: MedicalResult, id?: string) {
...
export async function listAssessments(uid: string, max = 20): Promise<RiskAssessment[]> {
  const snap = await getDocs(query(assessmentsCol(uid), orderBy("createdAt", "desc"), fbLimit(max)));
...
export async function listNotifications(uid: string, max = 50): Promise<AppNotification[]> {
  const snap = await getDocs(query(notificationsCol(uid), orderBy("createdAt", "desc"), fbLimit(max)));
...
export async function createSession(uid: string, s: AgentSession) {
  const ref = await addDoc(
    sessionsCol(uid),
    clean({ ...s, createdAt: serverTimestamp(), startedAt: serverTimestamp(), lastActivityAt: serverTimestamp() }),
  );
  return ref.id;
}
...
export async function listSessions(uid: string, max = 25): Promise<AgentSession[]> {
  const snap = await getDocs(query(sessionsCol(uid), orderBy("lastActivityAt", "desc"), fbLimit(max)));
  return snap.docs.map((d) => map<AgentSession>(d));
}

export async function addMessage(uid: string, sessionId: string, m: AgentMessage) {
  const ref = await addDoc(messagesCol(uid, sessionId), clean({ ...m, timestamp: serverTimestamp() }));
  return ref.id;
}

/** Bounded history: never load an unlimited conversation. */
export async function listMessages(uid: string, sessionId: string, max = 50): Promise<AgentMessage[]> {
  const snap = await getDocs(query(messagesCol(uid, sessionId), orderBy("timestamp", "asc"), fbLimit(max)));
...
function collectionRef(uid: string, name: string) {
  return query(
    ...
    (({ dailyCheckins: checkinsCol, healthRecords: recordsCol, riskAssessments: assessmentsCol, goals: goalsCol, notifications: notificationsCol, specialistGuidance: guidanceCol, supportRequests: supportCol } as any)[name])(uid),
  );
}
