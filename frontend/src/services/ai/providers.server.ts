...
async function callProvider(cfg: ProviderConfig, req: AIRequest): Promise<string> {
  ...
  try {
    ...
    if (!res.ok) {
      const retryable =
        res.status === 429 || res.status >= 500 || res.status === 402 || res.status === 401;
      throw new ProviderError(`${cfg.name}: HTTP ${res.status}`, retryable);
    }
    const data = (await res.json()) as { choices?: Array<{ message?: { content?: string } }> };
    const content = data.choices?.[0]?.message?.content;
    if (!content || typeof content !== "string")
      throw new ProviderError(`${cfg.name}: malformed response`, true);
    return content;
  } catch (e) {
    if (e instanceof ProviderError) throw e;
    throw new ProviderError(
      `${cfg.name}: ${(e as Error).name === "AbortError" ? "timeout" : "network error"}`,
      true,
    );
  } finally {
    clearTimeout(timer);
